...
self-explanatory. We defined \hs{SymbolicValue} and \hs{SymbolicArray} type
...
Registers and memory store 64-bit data values (\hs{Int64}). There are~4
registers (addressed by \hs{Word2}) and 256 memory locations (addressed by
\hs{Word8}). The register bank and memory are represented by symbolic arrays
that can be accessed via SBV's functions \hs{readArray} and \hs{writeArray}.

% \subsubsection{Instructions and programs}
REDFIN uses 16-bit \hs{InstructionCode} to encode instructions, whose 6
...
The~\hs{halt} instruction sets the flag~\hs{Halt}, thereby stopping the
execution of the current subroutine until a new one is started by a higher-level
system controller that resets \hs{Halt}. The auxiliary function \hs{writeFlag}
...
\hs{State} as input, and executes $N$ instructions using the function
\hs{executeInstruction} defined above. In each \hs{state} we merge two possible
futures: (i) if the \hs{Halt} flag is set, we continue the
simulation from the \hs{next} state, and (ii) otherwise we remain in the current
