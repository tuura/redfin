...
--
-----------------------------------------------------------------------------
module Redfin.Examples.MotorControlLoopInvariant where
...
--  To execute simulation, type into ghci:
--  proveWith prover simulateMotorControl
verifyMotorControl :: Symbolic SBool
verifyMotorControl = do
    a_max <- forall "a_max"
    v_max <- forall "v_max"
    dist  <- forall "dist"
    s     <- forall "s"
    v     <- forall "v"
    constrain $ a_max .> 0 &&& a_max .<= 10^6
    constrain $ v_max .> 0 &&& v_max .<= 10^6
    constrain $ dist  .> 0
    constrain $ a_max .<= v_max
    constrain $ v .>= 0    &&& v .<= v_max
    -- constrain $ a .>= 0    &&& a .<= a_max
    constrain $ a_max .<= v_max

    -- To simulate a program with concrete values we need to init the memory
    -- with literal constants instead of symbolic variables, like that:
    let mem = initialiseMemory [ (0, a_max)   -- a_max = 2
                               , (1, v_max)  -- v_max = 30
                               , (2, dist)    -- dist  = 101
                               , (3, s)
                               , (4, v)
    ...
    let s_final = readArray (memory finalState) 3
        v_final = readArray (memory finalState) 4
        halted  = readArray (flags finalState) (flagId Halt)

    return $ v_final .<= v_max
            -- v_final .== 0 &&&
            -- halted
