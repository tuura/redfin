...
\noindent
We extract the computed \hs{result} and the value of the flag \hs{Halt} from the
\hs{finalState}, and then assert that the processor has \hs{halted}, and that
the \hs{result} is non-negative and is equal to that computed by the high-level
Haskell expression~\hs{energyEstimate}.
...
a direct unoptimised translation of the \hs{energyEstimate} expression into
...
\noindent
The \hs{equivalence} check succeeds and takes 11.5s.
