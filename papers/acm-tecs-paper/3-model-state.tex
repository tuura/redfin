...
simulation from the~\hs{future}~state.
...
of the~\hs{Redfin} monad that is augmented with non-determinism and tracking of the
...
case of the conditional jump instruction~\hs{jmpi_ct}: one with the jump performed
and another with it skipped. The path condition,
a value of type~\hs{SymbolicValue}~\hs{Bool},
...
The modification of the~\hs{executeInstruction} function makes the type and semantics
of~\hs{simulate} different too:
\begin{minted}[xleftmargin=10pt]{haskell}
...
all current REDFIN instructions, we decided to reuse the well-tested~\hs{Data.Tree}
Haskell library which implements the more general rose tree data structure. Here, we provide
a simplified definition for reference:~\hs{data Tree a = Node a [Tree a]}.}.
The simulation starts form an initial state with a path condition representing the
precondition of the program, which can be instantiated with~\hs{true} if no
precondition is imposed. Proceeding further, more states can stem from every successor
state, until either a~\hs{halt} command is encountered or the maximum amount of simulation
steps is reached.
