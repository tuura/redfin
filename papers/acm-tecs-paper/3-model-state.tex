...
execution trace where every node a program state
...
states. Note that since no merging is performed, the branching instructions,
such as \hs{jmpi_ct}, now generate two possible successor states.
This semantics is captured by the \hs{executeInstructionBranching} function of type
isomorphic to \hs{State}~\hs{->}~\hs{[State]}.
We now recursively call the~\hs{simulate} function with every child as the
initial state and attaching the results as leaves to the current node.


\begin{minted}[xleftmargin=10pt]{haskell}
simulate :: Int -> State -> Tree State
simulate steps state =
    let halted = readArray (flags state) (flagId Halt)
    in if steps <= 0 || halted
       then pure (Node [])
       else do futures <- traverse (simulate (steps-1))
                                   (executeInstructionBranching state)
               pure (Node state futures)
\end{minted}
